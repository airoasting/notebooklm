...
현재 들어 있는 "노트북LM으로 다 됨" 사양은 예시입니다. 자기 책에 맞게 아래 표를 보면서 섹션별로 교체하시면 됩니다.

## `user-book-toc.md` 채우기

이 한 파일이 책 전체의 설계도입니다. 6개 섹션으로 나뉘어 있고, 각 섹션의 역할과 교체 가이드는 다음과 같습니다.

| 섹션 | 무엇을 정의하는가 | 무엇을 바꿔야 하는가 |
|------|-----------------|---------------------|
| 1. 기본 정보 | 제목, 부제, 필명, 대상 독자, 독자 지칭, 판형, 목표 분량, 기준 연도, 환율, 장/활용법 제목 형식, 용어 통일 규칙 | 모든 항목을 자기 책의 메타데이터로 직접 입력합니다. 이 값들이 본문 톤·연도·표기에 그대로 반영됩니다 |
| 2. 페르소나 | 책의 화자가 누구인지(캐릭터, 말투, 일관성 원칙) | 캐릭터 설정과 말투를 새로 씁니다. 프롤로그부터 부록까지 한 목소리로 유지되므로 1~2문단으로 또렷하게 정의해 두는 것이 좋습니다 |
| 3. 목차 | 프롤로그 → N부(장 또는 활용법) → 에필로그 → 부록 → 참고문헌의 전체 구조와 각 부의 마무리 브릿지 | 부/장/활용법 제목과 부제, 부의 마무리 문장을 전부 자기 책에 맞게 다시 씁니다. 이 목차가 `/write` 단계에서 청크 분할 기준이 됩니다 |
| 4. 활용법 세부 가이드 | 특정 챕터에만 적용할 상세 작성 규칙(현재는 활용법 09 슬라이드) + 활용법 공통 작성 가이드(도입·핵심 개념·워크플로우·프롬프트·팁·정리 박스 6요소) | 특별히 강조할 챕터가 있으면 같은 형식으로 추가하고, 없으면 공통 작성 가이드만 남깁니다 |
| 5. 작성 스타일 | 문체, 어미(해라체), 금지 부호(em dash 등), 불릿 기호, 영어 표현 최소화 등 표기 규칙 | 다른 장르나 다른 언어로 책을 쓰신다면 이 섹션을 통째로 새로 씁니다. `verify.py`가 여기 정의된 규칙으로 원고를 자동 검사합니다 |
| 6. 퍼블리싱 사양 | Word 문서의 서식(폰트·색상·여백·표·이미지·핵심 정리 박스 등)과 이미지 생성 규칙 | 폰트(Pretendard 등)와 컬러 톤만 바꾸면 거의 그대로 사용할 수 있습니다. 판형이나 문서 구조를 크게 바꿀 때만 전반을 손봅니다 |

> `CLAUDE.md`, 스킬 4개(`/research`, `/write`, `/review`, `/publish`), `verify.py`는 책마다 손대지 않습니다. **유일하게 교체하는 파일은 `user-book-toc.md` 하나입니다.**
